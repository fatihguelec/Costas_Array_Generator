...
//to find (q-4) and (q-5)th order Costas Arrays with G4-G5 Method
...
//int num_costas = 0;//# of Costas arrays of (q-2)th order
int num_costas4 = 0;//# of Costas arrays of (q-4)th order
int num_costas5 = 0;//# of Costas arrays of (q-4)th order
...
int main ()
{
    ...
    out4 = fopen("G4.txt", "w");
    out5 = fopen("G5.txt", "w");
    printf("Enter a prime or prime power greater than 2\n");
    scanf ("%d",&q);
    ...
    fprintf(out,"Number of (%d)th order G4 Costas Arrays = %d\n", q-4,num_costas4);
    fprintf(out,"Number of (%d)th order G5 Costas Arrays = %d\n", q-5,num_costas5);
...
void costas(polynomial g, int p, int k, int q)
{
    ...
    for(i = 2; i < q; i++)
    {
        ...
        //If the first element of the Costas array is not 1 then pass to the other primitive root.
        if((costas_array[0] != 1))
        {
            costas_array[0] = 0;
            break;
    ...
    //Control and construct G4(q-4)th order Costas array if possible
    if((costas_array[0] == 1)&&(costas_array[q-3] == 2))
    {
        //Print (q-2)th 0rder Costas array
        ...
        fprintf(out4," and for beta = a\n(%d)th order G4 Costas Array :\n{", q-3);


        for(j = 1; j < q-3; j++)
        {
            fprintf(out4,"%d ", costas_array[j]-2);
        }
        fprintf(out4,"}\n");
        num_costas4++;


        //Control and construct G5(q-5)th order Costas array if possible
        if(costas_array[1] == q-2)
        {
            fprintf(out5,"\nFor primitive element:{");
            for (i=0;i<=k; i++)
            {
...
//            printf("\n");
        ...
        if(primitive == true)
        {
            ...
            if((costas_array[0] == 1)&&(costas_array[q-3] == 2))
            {
                //Print (q-2)th order Costas array
                fprintf(out3,"\nFor primitive element:{");
                for (i=0;i<=k; i++)
                {
                    fprintf(out3,"%d ", g.poly[i]);
