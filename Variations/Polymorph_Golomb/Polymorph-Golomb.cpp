...
//to find polymorphs of (q-2), (q-3) and (q-4)th order Costas Arrays with Golomb Method
...
FILE *out, *out2, *out3, *out4, *out5, *out6;
int num_costas = 0;//# of Costas arrays of (q-2)th degree
int num_costas3 = 0;//# of Costas arrays of (q-3)th degree
int num_costas4 = 0;//# of Costas arrays of (q-4)th degree
...
int main ()
{
    ...
    out4 = fopen("Golomb3.txt", "w");
    out5 = fopen("Golomb4.txt", "w");
    out6 = fopen("Polymorphs.txt", "w");
    ...
    fprintf(out,"Number of (%d)th order Costas Arrays = %d\n", q-2,num_costas);
    fprintf(out,"Number of (%d)th order Costas Arrays = %d\n", q-3,num_costas3);
    fprintf(out,"Number of (%d)th order Costas Arrays = %d\n", q-4,num_costas4);
...
void costas(polynomial g, int p, int k, int q)
{
    ...
    fprintf(out3,"}\n");
    num_costas++;
    find_polymorphs(q-2, costas_array);

    //Control and construct (q-3) and (q-4)th order Costas array if possible
    if(costas_array[0] == 1)
    {
        fprintf(out4,"\nFor primitive element:{");
        for (i=0;i<=k; i++)
        {
            fprintf(out4,"%d ", g.poly[i]);
        }
        fprintf(out4,"} = ");
        //Print primitive element as x^0+x^1+...+x^n
        for(j = k; j >= 0; j--)//find the biggest order coefficent index
        ...
        fprintf(out4," and for beta = a\n(%d)th order Costas Array :\n{", q-3);

        polymorph.resize(q-3);
        for(j = 1; j < q-2; j++)
        {
            fprintf(out4,"%d ", costas_array[j]-1);
            polymorph[j-1] = costas_array[j]-1;
        }
        fprintf(out4,"}\n");
        num_costas3++;
    ...
    if((costas_array[0] == 1) && (costas_array[1] == 2))
    {
        fprintf(out5,"\nFor primitive element:{");
        for (i=0;i<=k; i++)
        {
            fprintf(out5,"%d ", g.poly[i]);
        }
        fprintf(out5,"} = ");
        //Print primitive element as x^0+x^1+...+x^n
        for(j = k; j >= 0; j--)//find the biggest order coefficent index
        ...
        fprintf(out5," and for beta = a\n(%d)th order Costas Array :\n{", q-4);

        polymorph.resize(q-4);
        for(j = 2; j < q-2; j++)
        {
            fprintf(out5,"%d ", costas_array[j]-2);
            polymorph[j-2] = costas_array[j]-2;
        }
        fprintf(out5,"}\n");
        num_costas4++;
        find_polymorphs(q-4, polymorph);
    }
    ...
    for(r = 2; r <= q-2; r++)
    {
        ...
        //Control and construct (q-3)th degree Costas array if possible
        if(costas_array[0] == 1)
        {
            fprintf(out4,"\nFor primitive element:{");
            for (i=0;i<=k; i++)
...
void find_polymorphs(int n, vector<int> polymorph)
{
    ...
    fprintf(out6,"Polymorphs of the Costas Array = {");
    for(j=0;j<n;j++)
    {
        fprintf(out6,"%2d ",polymorph[j]);
    }
    fprintf(out6,"}\n");
    ...
    if(temp2 == polymorph)
    {
        fprintf(out6,"Diagonally Symmetric\n");
    }
    else
    {
        fprintf(out6,"{");
        for(i = 0; i < n; i++)
        {
            fprintf(out6,"%d ",polymorph[i]);
        }
        fprintf(out6,"} //Diagonal\n");
