...
//to find (q-4)th order Costas Arrays with Taylor Method
...
int phi (int i);
//void primitive_polynomial(int p, int k);
...
void costas(polynomial g, int p, int k, int q)
{
    ...
    //Control and construct (q-4)th order Costas array if possible
    if(costas_array[0] == 2 && costas_array[1] == 1)

    {
        ...
        for(j = 2; j < q-2; j++)
        {
            fprintf(out3,"%d ", costas_array[j]-2);
        }
        fprintf(out3,"}\n\n");
    }
    else
    {
        fprintf(out3,"(q-4)th order Costas Array can not be constructed.\n");
    }
