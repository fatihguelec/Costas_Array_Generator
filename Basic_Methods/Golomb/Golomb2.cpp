...
//to find (q-2), (q-3) and (q-4)th order Costas Arrays with Golomb Method
...
polynomial GF_poly_mul(polynomial f, polynomial g, int p);
...
void costas(polynomial g, int p, int k, int q)
{
    ...
    fprintf(out3,"} = ");
    //Print primitive element as x^0+x^1+...+x^n
    for(j = k; j >= 0; j--)//find the biggest order coefficient index
    {
        if(g.poly[j] != 0)
        {
            break;
